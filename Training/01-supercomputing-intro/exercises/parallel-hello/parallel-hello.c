...
{
  int mpi_tasks, rank, num_threads, thread_id, provided;
  cpu_set_t coremask;
...
#pragma omp single
  ...
  printf("Running with %d MPI tasks and %d OpenMP threads\n", mpi_tasks, num_threads);
  fflush(stdout);
...
#pragma omp barrier
    ...
    printf("Rank %03d thread %02d on %s, on core = %s (%f seconds).\n",
